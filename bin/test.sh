...
# Make tiles bed
bedtools makewindows -g $chr_sizes -w 100 -i 'winnum' > $OUTDIR/windows.bed
...
cat $aln | grep "\+$" | bedtools coverage -a $OUTDIR/tiles_int_targets.bed -b - | \
cut -f 1,2,3,8,9,10,11,12 >> $OUTDIR/positive_target_cov.bed
